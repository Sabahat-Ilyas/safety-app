    ...
    const handleSignup = async (e) => {
        ...
        try {
            ...
            const response = await fetch(`${API_BASE_URL}/api/signup`, {
                ...
                body: JSON.stringify({ name: name.trim(), email: normalizedEmail, password: password.trim() }),
            });

            const data = await response.json();

            if (response.ok) {
                // Automatically login after signup might be better, 
                // but for now let's just navigate to login or home
            ...
            <div className="relative z-10 flex-1 flex flex-col px-8 pb-12 pt-10 min-h-[600px]">
                ...
                <div className="flex-1 flex flex-col items-center justify-center space-y-6">
                    ...
                    <div className="text-center space-y-2">
                        <h1 className="text-4xl font-black tracking-tighter bg-gradient-to-r from-white via-red-100 to-white bg-clip-text text-transparent">CREATE ACCOUNT</h1>
                        <p className="text-red-400/80 text-base font-medium tracking-wide uppercase">Join Guardian today</p>
                    </div>
                </div>
                ...
                <div className="w-full max-w-sm mx-auto space-y-6 animate-fade-in-up">
                    ...
                    <div className="flex justify-center text-sm text-white/40 px-2 font-sans">
                        <span className="mr-1">Already have an account?</span>
                        <button onClick={() => navigate('/')} className="text-white hover:underline transition-colors">Login</button>
                    </div>
